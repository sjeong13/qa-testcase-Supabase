...
#2025-11-26 : 벡터 검색 2회 동시 실행(병렬) =====> 실행해보았으나 큰 효과를 체감하지 못함
                ...
                with st.spinner("AI가 유사한 테스트 케이스를 벡터 검색 중..."):
                    ...
                    if client:
                        ...
                        try:
                            ...
                            with st.spinner("벡터가 병렬로 검색 중..."):
                                # 병렬로 2개 검색 동시 실행 (test_cases + spec_docs)
                                with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                                    ...
                                    # 결과 기다리기
                                    relevant_cases = future_cases.result()
                                    spec_docs = future_docs.result()
                            ...
                            # 2. 기획 문서도 벡터 검색
                            spec_docs_str = ""
                            # spec_docs = search_similar_spec_docs(query=search_query, limit=50)
                        ...
                        # 4. AI 프롬프트 (기존과 동일)
                        prompt = f"""[역할 부여]
너는 IT 노코드 웹 빌더 SaaS에 다니고 있는 꼼꼼한 QA 전문가야.
(1) 테스트 설계, 테스트 케이스 작성, 자동화 업무 수행
(2) 서비스 안정성 기여. 리그레이션을 중심 업무 수행
...
1. **직접 관련된** 테스트 케이스 찾기
2. 기획 문서를 참고하여 기능의 의도와 맥락을 파악하고 의존 기능 추론하기
3. 논리적인 순서로 체크리스트 만들기
4. **반드시 위 표 양식으로 신규 테스트 케이스들을 생성하기. NO 1부터 번호 시작하기**
5. **existing_test_cases의 id는 반드시 숫자여야 함. 학습 데이터의 id 필드를 참조하기**
...
{{
  ...
  "additional_suggestions": "핵심 edge case 제안"
}}
```
